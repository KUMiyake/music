...
public final class MonitorRecorder {
    ...
    public static JFreeChart GenerateSpectrogramMonitorChart(Recorder recorder)
            throws IOException,
    ...
    /* シフト長＝フレーム長の8分の1 */
        ...
        final int bins = (int)(fftSize * freqMax / recorder.getSampleRate());
        final double[] window =
        ...
        /* Playerの再生停止に合わせて描画も停止 */
                    if (!recorder.isOpen()) executor.shutdown();
                },
      /* initialDelay = */ 0L,
